 function [model,enzUsages,modifications,GAM,massCoverage] = constrainEnzymes(model,f,raw,GAM,Ptot,pIDs,data,gRate,c_UptakeExp)
% constrainEnzymes
%
...
%   raw            (Opt) If true the biomass will not be refitted
...
%Refit biomass if raw is not set
if nargin <3
    raw=false;
end
...
Pbase = sumProtein(model); %Why is it not Ptot at this place
if Pmeasured > 0
...
fprintf(' Done!\n')
if sum(data)==0 && ~raw %Why is this only done if no proteomics are given?
