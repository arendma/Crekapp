...
operation = 'list/';
gene_list = webread([base operation keggID]);
...
operation = 'get/';
if numel(gene_id)>20000
    warning('Not all gene infos were downloaded from kegg because the list of genes was to large')
end
for i = 1:min([numel(gene_id),20000])
    try
        gene = webread([base operation keggID ':' gene_id{i}{1}]);
